...
3. If a complex task arises, summon the Neocortex using `_summon_neocortex_tool`."""
    ...
    async def _summon_neocortex_tool(self, tool_context, task: str) -> str:
        ...
        result = await runner.run_async(new_message=task)
    ...
    async def run_proactive_loop(self, context_stream):
        ...
        runner = Runner(agent=self.slm, session_service=self.session_service)
        
        # Create session with initial state
        session_id = "saguaro_session"
        self.session_service.create_session(session_id=session_id, state=self.initial_state)
        ...
        async for content in context_stream:
            try:
                ...
                await runner.run_async(new_message=content, session_id=session_id)
